...
class DatabasesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create database" do
    assert_difference('Database.count') do
      post databases_url, params: { database: { email: @database.email, gender: @database.gender, lastname: @database.lastname, name: @database.name } }, as: :json
    end
  ...
  test "should update database" do
    patch database_url(@database), params: { database: { email: @database.email, gender: @database.gender, lastname: @database.lastname, name: @database.name } }, as: :json
    assert_response 200
  end
